...
chapter_order: 1
section_order: 4
permalink: /materi-algebrica/sets-and-numbers/integers/
---

## Definition

Among the different [types of numbers](https://algebrica.org/types-of-numbers/), the integers emerge when we extend the [natural numbers](https://algebrica.org/natural-numbers) to include the additive opposites of every positive quantity. In this enlarged system we find all whole quantities, both positive and negative, together with zero. The [set](https://algebrica.org/sets/) is denoted by $\mathbb{Z}$. Symbolically we write:
$$\mathbb{Z} = \ldots , - 3 , - 2 , - 1 , 0 , 1 , 2 , 3 , \ldots$$
an infinite collection of evenly spaced points along the number [line](https://algebrica.org/lines).

![Integers.](https://algebrica.org/wp-content/uploads/resources/images/integers.png "Integers.")

A rigorous construction models each integer as a class of ordered pairs of natural numbers. Take pairs $\left(\right. a , b \left.\right)$ with $a , b \in \mathbb{N}$ and say that two pairs belong to the same class whenever:
$$\left(\right. a , b \left.\right) sim \left(\right. c , d \left.\right) \leftrightarrow a + d = b + c$$

The pair $\left(\right. a , b \left.\right)$ represents the net difference between $a$ and $b$:
...
For example, consider the pair of natural numbers $\left(\right. 2 , 5 \left.\right) .$ In the construction that builds the integers from ordered pairs, this element represents the net amount obtained by comparing its two components. Since the second entry is larger than the first, the pair corresponds to a negative integer:
$$2 - 5 = - 3$$
...
For instance, the pair $\left(\right. 4 , 7 \left.\right)$ lies in the same class as $\left(\right. 2 , 5 \left.\right)$, because:
$$4 + 5 = 9 \text{and} 7 + 2 = 9$$
...
When we say that the integers form a [ring](https://algebrica.org/rings/), we mean that the set $\mathbb{Z}$ comes equipped with two operations, addition and multiplication, that interact in a structured and predictable way. This structure ensures that arithmetic with integers behaves consistently, no matter how large or small the numbers involved may be.

The ring axioms for $\left(\right. \mathbb{Z} , + , \cdot \left.\right)$ are the following. For all $a , b , c \in \mathbb{Z}$:

- Closure: the sum and product of any two integers are again integers: $a + b \in \mathbb{Z}$ and $a b \in \mathbb{Z} .$
- Associativity: both operations are associative: $a + \left(\right. b + c \left.\right) = \left(\right. a + b \left.\right) + c$ and $a \left(\right. b c \left.\right) = \left(\right. a b \left.\right) c .$
- Identity elements: there exist neutral elements for both operations: $a + 0 = a$ and $a \cdot 1 = a .$
- Additive inverses: every integer has an opposite: $a + \left(\right. - a \left.\right) = 0.$
- Commutativity of addition: the order of summands does not affect the result: $a + b = b + a .$
- Distributivity: multiplication distributes over addition: $a \left(\right. b + c \left.\right) = a b + a c .$

Multiplication in $\mathbb{Z}$ is also commutative, that is, $a b = b a$ for all $a , b \in \mathbb{Z}$, which makes $\left(\right. \mathbb{Z} , + , \cdot \left.\right)$ a commutative ring. Note that integers do not possess multiplicative inverses in general: the only integers $a$ for which $a^{- 1} \in \mathbb{Z}$ are $a = 1$ and $a = - 1$, which is why $\mathbb{Z}$ is a ring but not a field.
...
$$a \left(\right. b + c \left.\right) = a b + a c$$
...
Modular arithmetic describes how integers behave when we are interested only in their remainders after division by a fixed integer $n$. Within $\mathbb{Z}$, two integers are said to be equivalent [modulo](https://algebrica.org/modulo-operator/) $n$ when they differ by a multiple of $n$. For example, in arithmetic modulo $12$, the integers $14$ and $2$ represent the same residue class because $14 - 2 = 12$. Addition and multiplication are carried out as usual, but the final result is replaced by its remainder upon division by $n$. For example:

$$7 + 9 \equiv 4 \left(\right. mod 12 \left.\right)$$
$$5 \times 7 \equiv 11 \left(\right. mod 12 \left.\right)$$
...
- $p \left(\right. n \left.\right) \rightarrow p \left(\right. n + 1 \left.\right) \forall n \in \mathbb{N} .$ If $n \in A$, then $n + 1 \in A .$

Thus, $p \left(\right. n \left.\right)$ is true for every $n \in \mathbb{N} .$ A concrete illustration shows how this carries over to the integers. Consider the claim that the sum of the first $n$ positive integers equals:
...
binary systemdecimal systemorderring structureadditive inverseidentitiesdistributivitycommutativityassociativityclosuremultiplicationadditionadditive oppositesequivalence relationconstruction pairsnumber linenotation Zextension of naturalsdefinitionstructuresoperationsfoundations
