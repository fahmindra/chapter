...
chapter_order: 1
section_order: 7
permalink: /materi-algebrica/sets-and-numbers/properties-of-real-numbers/
---

## Order of operations

Before discussing the algebraic properties of [real numbers](https://algebrica.org/real-numbers), it is essential to clarify how operations are performed in an expression. The fundamental properties of real numbers allow us to manipulate expressions, but the order of operations ensures that every expression has a well-defined and unambiguous value. When an expression involves several operations at once, we follow a precise and universally accepted sequence. A commonly used mnemonic is PEMDAS:
...
- Consider now $\left(\right. 3 + 4 \left.\right) \cdot 2$. Parentheses alter the natural priority: $3 + 4 = 7$ then $7 \cdot 2 = 14$.
...
The real numbers are said to be dense: between any two distinct real numbers, there always exists another. Formally, for every $a , b \in \mathbb{R}$ with $a < b$, there exists $c \in \mathbb{R}$ such that $a < c < b$. One natural choice is the [arithmetic mean](https://algebrica.org/arithmetic-mean/) $c = \left(\right. a + b \left.\right) / 2$, but infinitely many such values exist.
...
In plain terms: when you add or multiply two real numbers, you always get a real number back. The result never escapes the set. This may sound like a trivial observation but closure is far from automatic when working with other collections of numbers. The [integers](https://algebrica.org/integers), for instance, are closed under addition and multiplication, but division lets you out: $1 \div 2$ is not an integer. Go one step further and subtract within the [natural numbers](https://algebrica.org/natural-numbers/): $3 - 5$ has no answer there. Closure is therefore not a birthright of every number set, but something that has to be earned, checked operation by operation, set by set.
...
> More generally, in algebraic manipulation, commutativity allows us to reorder terms within sums or products. For example $2 x + 5 = 5 + 2 x$ which may be useful when grouping like terms or arranging expressions into a standard form.
...
$$a + \left(\right. b + c \left.\right) = \left(\right. a + b \left.\right) + c$$

$$a \cdot \left(\right. b \cdot c \left.\right) = \left(\right. a \cdot b \left.\right) \cdot c$$
...
$$a - \left(\right. b - c \left.\right) \neq \left(\right. a - b \left.\right) - c$$

$$a \div \left(\right. b \div c \left.\right) \neq \left(\right. a \div b \left.\right) \div c$$
...
$$2 + \left(\right. 3 + 4 \left.\right) = 2 + 7 = 9$$

$$\left(\right. 2 + 3 \left.\right) + 4 = 5 + 4 = 9$$

For multiplication:

$$2 \cdot \left(\right. 3 \cdot 4 \left.\right) = 2 \cdot 12 = 24$$

$$\left(\right. 2 \cdot 3 \left.\right) \cdot 4 = 6 \cdot 4 = 24$$
...
$$a \cdot \left(\right. b + c \left.\right) = a \cdot b + a \cdot c$$

$$\left(\right. b + c \left.\right) \cdot a = b \cdot a + c \cdot a$$
...
$$3 \cdot \left(\right. 4 + 5 \left.\right) = 3 \cdot 9 = 27$$

Now compute the right-hand side:

$$3 \cdot 4 + 3 \cdot 5 = 12 + 15 = 27$$

Both expressions yield the same result, confirming the distributive property. The distributive property is fundamental in algebraic manipulation. It allows us to expand expressions such as $a \left(\right. x + y \left.\right)$ into $a x + a y$ and conversely, to factor expressions like $a x + a y$ into $a \left(\right. x + y \left.\right)$.
...
The number $0$ is called the additive identity because adding zero does not alter the value of a real number. The uniqueness of this element is important. If a number $n$ satisfies $a + n = a \forall a \in \mathbb{R}$ then necessarily $n = 0$.
...
The number $1$ is called the multiplicative identity because multiplying by one leaves every real number unchanged. As in the additive case, this identity element is unique. If a number $n$ satisfies $a \cdot n = a \forall a \in \mathbb{R}$ then necessarily $n = 1$.
...
For every nonzero real number $a \in \mathbb{R}$, there exists a unique real number $\frac{1}{a}$ such that:

$$a \cdot \frac{1}{a} = 1$$

The number $\frac{1}{a}$ is called the multiplicative inverse (or reciprocal) of $a$. It is defined only for $a \neq 0$, since no real number multiplied by $0$ can produce the multiplicative identity $1$. As in the additive case, the multiplicative inverse is unique. If a number $b$ satisfies $a \cdot b = 1$ then necessarily $b = \frac{1}{a}$.

completenessdensityinequalitiesorderinverseidentitydistributivityassociativitycommutativityclosureexponentsdivisionsubtractionmultiplicationadditionorder of operationsorder and structurealgebraic propertiesoperations
