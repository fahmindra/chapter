...
![](https://algebrica.org/wp-content/uploads/resources/images/reduction-formulas-1-1.png)
...
$$tan ⁡ \left(\right. \frac{\pi}{2} + \alpha \left.\right) & = \frac{cos ⁡ \alpha}{- sin ⁡ \alpha} = - cot ⁡ \alpha \\ cot ⁡ \left(\right. \frac{\pi}{2} + \alpha \left.\right) & = \frac{- sin ⁡ \alpha}{cos ⁡ \alpha} = - tan ⁡ \alpha$$
...
![](https://algebrica.org/wp-content/uploads/resources/images/reduction-formulas-2-2.png)
...
$$sin ⁡ \left(\right. \pi + \alpha \left.\right) & = - sin ⁡ \alpha \\ cos ⁡ \left(\right. \pi + \alpha \left.\right) & = - cos ⁡ \alpha$$
...
$$tan ⁡ \left(\right. \pi + \alpha \left.\right) & = \frac{- sin ⁡ \alpha}{- cos ⁡ \alpha} = tan ⁡ \alpha \\ cot ⁡ \left(\right. \pi + \alpha \left.\right) & = \frac{- cos ⁡ \alpha}{- sin ⁡ \alpha} = cot ⁡ \alpha$$
...
![](https://algebrica.org/wp-content/uploads/resources/images/reduction-formulas-4.png)
...
![](https://algebrica.org/wp-content/uploads/resources/images/reduction-formulas-5.png)

A direct reading of the coordinates of the terminal side on the unit circle gives the following identities for sine and cosine:
$$sin ⁡ \left(\right. \frac{3 \pi}{2} + \alpha \left.\right) & = - cos ⁡ \alpha \\ cos ⁡ \left(\right. \frac{3 \pi}{2} + \alpha \left.\right) & = sin ⁡ \alpha$$
...
$$tan ⁡ \left(\right. \frac{3 \pi}{2} + \alpha \left.\right) & = \frac{- cos ⁡ \alpha}{sin ⁡ \alpha} = - cot ⁡ \alpha \\ cot ⁡ \left(\right. \frac{3 \pi}{2} + \alpha \left.\right) & = \frac{sin ⁡ \alpha}{- cos ⁡ \alpha} = - tan ⁡ \alpha$$
...
![](https://algebrica.org/wp-content/uploads/resources/images/reduction-formulas-6.png)
...
The angle $2 \pi - \alpha$ is coterminal with $- \alpha$, that is, the two angles differ by an integer multiple of $2 \pi$ and therefore identify the same point on the unit circle. Since the trigonometric functions have period $2 \pi$, they take the same values at $2 \pi - \alpha$ and at $- \alpha$, and this equivalence will be used implicitly in the identiti

![](https://algebrica.org/wp-content/uploads/resources/images/reduction-formulas-7.png)

A direct reading of the coordinates of the terminal side on the unit circle gives the following identities for sine and cosine:
$$sin ⁡ \left(\right. 2 \pi - \alpha \left.\right) & = - sin ⁡ \alpha \\ cos ⁡ \left(\right. 2 \pi - \alpha \left.\right) & = cos ⁡ \alpha$$
...
$$tan ⁡ \left(\right. 2 \pi - \alpha \left.\right) & = \frac{- sin ⁡ \alpha}{cos ⁡ \alpha} = - tan ⁡ \alpha \\ cot ⁡ \left(\right. 2 \pi - \alpha \left.\right) & = \frac{cos ⁡ \alpha}{- sin ⁡ \alpha} = - cot ⁡ \alpha$$
...
This perspective has concrete consequences in several directions. In the solution of [trigonometric equations](https://algebrica.org/trigonometric-equations/), reduction formulas are what allows one to reduce any equation involving $sin ⁡ \theta$, $cos ⁡ \theta$, $tan ⁡ \theta$ or $cot ⁡ \theta$ to a problem on an acute reference angle, and then to recover the full set of solutions by exploiting periodicity. In [integral calculus](https://algebrica.org/indefinite-integrals/), they are routinely used to simplify integrands of the form $sin ⁡ \left(\right. k \pi \pm x \left.\right)$ or $cos ⁡ \left(\right. k \pi \pm x \left.\right)$ before applying standard integration techniques. In the study of [Fourier series](https://algebrica.org/fourier-series/), the same sign rules underlie the classification of a function as even, odd, or neither, and determine which coefficients of the series vanish. From a more advanced standpoint, the reduction formulas anticipate the general addition formulas:

$$sin ⁡ \left(\right. \alpha + \beta \left.\right) & = sin ⁡ \alpha cos ⁡ \beta + cos ⁡ \alpha sin ⁡ \beta \\ cos ⁡ \left(\right. \alpha + \beta \left.\right) & = cos ⁡ \alpha cos ⁡ \beta - sin ⁡ \alpha sin ⁡ \beta$$
...
tan cot relationssin cos relationseven odd functionsrotation 2πreflection symmetrycoterminal anglesangle reductioncofunction swap2π − α3π/2 ± απ ± απ/2 ± αperiodicitysign rulesquadrantsacute angle αangle θunit circlereference anglestructuretransformationsfoundations
